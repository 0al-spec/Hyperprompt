...
| CI-02 | Define workflow triggers (PR to default branch with path filters for source code/GitHub Actions files, push to default with same filters, manual dispatch) | High | 0.5h | CI-01 | Dev **INPROGRESS** | `.github/workflows/ci.yml` contains required triggers and path filters |
