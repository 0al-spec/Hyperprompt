...
| CI-02 | Define workflow triggers (PR to default branch, push to default, manual dispatch) with path filters for source code and `.github` changes | High | 0.5h | CI-01 | Dev | `.github/workflows/ci.yml` contains required triggers and path filters to avoid running on docs-only changes |
...
| CI-04 | Add static analysis step conditioned on available scripts (e.g., `npm run lint`, `pytest --lint`, etc.) | Medium | 1h | CI-03 | Dev | Step skips with message if script missing; fails on non-zero exit | 
