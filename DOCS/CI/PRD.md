...
1. **Triggers**
   - On pull requests targeting default branch **only when source code or GitHub Actions files change** (e.g., `src/**/*`, `*.py`, `.github/workflows/**`).
...
2. **Job Stages (Linux)**
   ...
   - **Static Analysis:** optional linters/formatters; run only if defined scripts exist (e.g., `ruff check`, `flake8`, `pylint`, `black --check`, or `npm run lint`).
