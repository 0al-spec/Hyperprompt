...
The compiler shall accept a single Hypercode file as its primary input. The parser shall recognize lines matching the pattern of optional leading whitespace followed by a quoted string literal. Each indentation level shall consist of exactly four space characters. Tab characters in indentation shall be rejected with a diagnostic error indicating the line number and suggesting conversion to spaces.

The parser shall extract the content between the first and last double quotation mark on each non-empty, non-comment line. Lines beginning with the hash character (#) after optional whitespace shall be treated as comments and ignored. Empty lines shall be preserved in the AST as structural separators but shall not generate output.
...
A Hypercode source file consists of a sequence of lines. Each line is classified as blank, comment, or node. Blank lines contain only whitespace characters (space or tab). Comment lines begin with optional whitespace followed by the hash character. Node lines contain optional leading spaces for indentation, an opening double quotation mark, literal content, and a closing double quotation mark.
...
comment     = { space }, "#", { any-char }, newline ;
...
|-------|----------|--------------|-------------------|
|1.0.0  |2025-11-25|Egor Merkushev|Initial PRD release|
