...
final class MarkdownEmitterSourceMapTests: XCTestCase {
    func testEmissionProducesExactLineOriginsInOnePass() throws {
        let root = Node(
            ...
            depth: 0,
            location: SourceLocation(filePath: "/workspace/root.hc", line: 1),
            resolution: .inlineText
        )
        let markdown = Node(
            literal: "body.md",
            depth: 1,
            location: SourceLocation(filePath: "/workspace/root.hc", line: 2),
        ...
        let sibling = Node(
            literal: "Appendix",
            depth: 1,
            location: SourceLocation(filePath: "/workspace/root.hc", line: 3),
            resolution: .inlineText
        )
